...
class MedicamentoTratamiento extends Modelo {
  #tableName = "medicamentos_tratamiento";
  ...
  async create(medicamentoTratamiento) {
    try {
      const idCreado = await super.create(this.#tableName, medicamentoTratamiento);
      if (idCreado) {
        return await this.getById(idCreado);
      }
      return null;
    } catch (error) {
      throw new Error(`Error al crear el medicamento de tratamiento: ${error.message}`);
    }
  }
