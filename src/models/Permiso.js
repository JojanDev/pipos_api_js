...
class Permiso extends Modelo {
  ...
  async getById(id) {
    try {
      ...
      throw new Error(
        `Error al obtener el permiso con ID ${id}: ${error.message}`
      );
    }
  }
  ...
  async create(permiso) {
    try {
      const idCreado = await super.create(this.#tableName, permiso);
      if (idCreado) {
  ...
  async update(id, permiso) {
    try {
      if (await super.update(this.#tableName, id, permiso)) {
        return await this.getById(id);
      }
      return null;
    } catch (error) {
      throw new Error(
        `Error al actualizar el permiso con ID ${id}: ${error.message}`
      );
    }
  }
  ...
  async delete(id) {
    try {
      ...
      throw new Error(
        `Error al eliminar el permiso con ID ${id}: ${error.message}`
      );
    }
  }
}
